...
class MongoExtractorFactory:
    ...
    class _extractor:
        def __init__(self, collection_name: str, wrapper_model: DeclarativeBase, database: Database, batch_size: int, with_id: bool):
            self.col = collection_name
            self.wrapper = wrapper_model
            self.cursor = database[collection_name].find({}, {"_id":int(with_id)}, batch_size = batch_size)
    ...
    def __call__(self, collection_name: str, wrapper_model: DeclarativeBase, batch_size : int = 100, with_id=False):
        if batch_size <= 0:
            raise Exception(f"Batch size must be > 0 (current: {batch_size})")
        return self._extractor(collection_name, wrapper_model, self.db, batch_size, with_id)
