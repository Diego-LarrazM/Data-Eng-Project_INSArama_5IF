...
class ReviewerDimORM(ModelsBase):
  ...
  # Fields
  ReviewerUsername = Column(String, nullable=True) # not all reviewers post their name
  IsCritic = Column(Boolean, nullable=False)
  Association = Column(String, nullable=True) # might be a user, non-critic

  def __repr__(self):
    return (f"<ReviewerDimORM(Reviewer_ID={self.Reviewer_ID}, ReviewerUsername='{self.ReviewerUsername}', "
